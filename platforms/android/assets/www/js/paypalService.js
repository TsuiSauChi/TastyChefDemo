  ...
  .factory('PaypalFactory', ['$q', '$filter', '$timeout', '$ionicPlatform', 'APP_CONSTS', function ($q, $filter, $timeout, $ionicPlatform, APP_CONSTS) {
    ...
    */
    that.initPaymentUI = function () {
      ...
      $ionicPlatform.ready().then(function () {
        ...
        PayPalMobile.prototype.init(clientIDs, that.onPayPalMobileInit);
      });
    ...
    */
    ...
    that.onPayPalMobileInit = function () {
      $ionicPlatform.ready().then(function () {
        PayPalMobile.prototype.prepareToRender(APP_CONSTS.payPalEnv, configuration(), function () {
          $timeout(function () {
            init_defer.resolve();
          });
        });
    ...
    */
    that.makePayment = function (total, name) {
      ...
      $ionicPlatform.ready().then(function () {
        PayPalMobile.prototype.renderSinglePaymentUI(createPayment(total, name), function (result) {
          $timeout(function () {
    ...
    */
    that.makeFuturePayment = function (total, name) {
      var defer = $q.defer();
      $ionicPlatform.ready().then(function () {
        PayPalMobile.prototype.renderFuturePaymentUI(
